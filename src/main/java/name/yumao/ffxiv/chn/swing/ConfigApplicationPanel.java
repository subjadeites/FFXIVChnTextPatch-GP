...
public class ConfigApplicationPanel extends JFrame implements ActionListener {
	...
	private JCheckBox rFont = new JCheckBox("替換字體");
	private JCheckBox rText = new JCheckBox("替換文本");
	...
	public ConfigApplicationPanel() {
		...
		setBounds((this.dimension.width - 290) / 2, (this.dimension.height - 190) / 2, 290, 220);
		setResizable(false);
		...
		this.pathField.setText(Config.getProperty("GamePath"));
		...
		this.fLangLableVal.setSelectedItem(Language.getNameByLang(Config.getProperty("FLanguage")));
		...
		this.rFont.setFont(new Font("Microsoft Yahei", 1, 12));
		this.rFont.setSelected(Config.getProperty("ReplaFont").equals("1"));
		add(this.rFont, 0);
		this.rText.setBounds(180, 160, 80, 20);
		this.rText.setFont(new Font("Microsoft Yahei", 1, 12));
		this.rText.setSelected(Config.getProperty("ReplaText").equals("1"));
		add(this.rText, 0);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == this.configButton) {
			...
			if (isFFXIVFloder(path)) {
				...
				Config.setProperty("ReplaFont", (this.rFont.isSelected() == true) ? "1" : "0");
				Config.setProperty("ReplaText", (this.rText.isSelected() == true) ? "1" : "0");
